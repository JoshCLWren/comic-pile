...
- Remote Redis caching is now disabled unless explicitly enabled, so production and Preview requests avoid Upstash startup calls, quota use, and cache invalidation traffic while the cache design is being repaired ([#873](https://github.com/JoshCLWren/comic-pile/pull/873)).
