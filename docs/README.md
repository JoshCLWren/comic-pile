...
Remote Redis caching is currently governed by the active cache issues and implementation. Documentation about caching must describe the current code rather than older provider experiments.
...
`scripts/check_markdown_docs.py` is the repository-side guard for local Markdown links and undocumented root-level Markdown sprawl. The #879 migration will use this hub as the destination for the final file inventory and will retire duplicate indexes once every tracked Markdown file has an explicit disposition.

Historical implementation notes and explorations should live under `archive/` or in the Wiki, not beside current operational instructions. Git history remains the source for deleted obsolete documentation.
