...
- INCIDENT (2026-09-06): GitHub OmniRoute Entry is intentionally disabled (`FACTORY_OMNIROUTE_ENABLED=off` by default) after repeated free-pool instability. `auto/coding:free`, `auto/reasoning:free`, and `auto/best-free` are all OmniRoute routes — the disable covers every one of them. Do not call OmniRoute, smoke it, or fall back across its auto selectors while the flag is off. The temporary `auto/best-free` capacity bridge stays removed. Re-enable only after Josh confirms gateway health by setting `FACTORY_OMNIROUTE_ENABLED=on`.
