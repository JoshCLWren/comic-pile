...
async def commit_existing_cbl_entries_to_reading_plan(
    ...
    *,
    ...
    Strict plans are rejected in this slice because their compiler currently
    lives in the API router. Missing physical comics are also rejected until the
    safe materializer is extracted from the archived #2161 implementation.
    Both boundaries fail before the plan is mutated.
    """
    await db.execute(
    ...
    selected_missing = [
        ...
        if entry.get("adoption_class") == "missing_importable"
    ]
    if selected_missing:
        raise CBLPlanAdoptionError(
    ...
    for entry in selected:
        ...
        node_id = f"cbl-{list_id}-issue-{issue_id_value}"
