"""Authenticated canonical Reading Plan CRUD and compatibility endpoints."""
...
from app.continuity_plan_readiness import evaluate_plan_readiness
...
from app.services.continuity_plan_writer import (
    apply_continuity_plan_write,
    plan_marker,
    replace_compiled_plan_rules,
    validate_plan_node_ownership,
)
...
def _marker(plan_id: int) -> str:
    """Compatibility wrapper for the canonical plan-rule marker."""
    return plan_marker(plan_id)
...
async def _validate_node_ownership(
    ...
    """Compatibility wrapper around shared Reading Plan ownership validation."""
    await validate_plan_node_ownership(db, user_id=user_id, nodes=nodes)
...
async def _replace_compiled_rules(
    ...
    *,
    ...
    """Compatibility wrapper around the shared Reading Plan rule compiler."""
...
async def create_continuity_plan(
    ...
    """Create a plan and compile only the hard semantics explicitly represented by it."""
    await validate_plan_node_ownership(db, user_id=current_user.id, nodes=payload.nodes)
    plan = ContinuityPlan(
    ...
    await db.flush()
    try:
        await replace_compiled_plan_rules(
            db,
            user_id=current_user.id,
            plan=plan,
            payload=payload,
...
async def update_continuity_plan(
    ...
    """Replace a plan atomically through the shared canonical Reading Plan writer."""
    plan = await _get_owned_plan(db, current_user.id, plan_id)
    try:
        await apply_continuity_plan_write(
            db,
            user_id=current_user.id,
            plan=plan,
            payload=payload,
...
@router.post(
    ...
    description=(
        ...
        "The source reading order is not mutated; the new plan is the canonical owner of "
        "the ordering intent. See docs/READING_PLAN_CANONICAL_MODEL.md."
    ),
)
...
async def delete_continuity_plan(
    ...
    await db.execute(
        delete(ContinuityRule).where(
            ...
            ContinuityRule.note == plan_marker(plan.id),
        )
    )
