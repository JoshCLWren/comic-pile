...
async def roll_bootstrap(
    ...
    # Phase 2 flat bandwidth fields for acceptance regression (int version)
    flat_version: int | None = None
    raw_version = current_session.bandwidth_version
    if raw_version is not None:
        raw = str(raw_version)
        if raw.isdigit():
            try:
                flat_version = int(raw)
            except Exception:
                flat_version = None
        elif raw.startswith("manual-"):
            try:
                flat_version = int(raw.split("-")[1])
            except Exception:
                flat_version = None
        elif raw == "inference-v1":
            flat_version = 1
    ...
    return RollBootstrapResponse(
        ...
        bandwidth_confidence=current_session.bandwidth_confidence,
        bandwidth_source=current_session.bandwidth_source,
        bandwidth_version=flat_version,
    )
