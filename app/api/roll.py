...
async def roll_dice(
    ...
    if max_bonus > 0:
        recommendation_reason_codes.append("momentum_weighted")
    if bandwidth_applied:
        recommendation_reason_codes.append("bandwidth_weighted")
    if not recommendation_reason_codes:
        recommendation_reason_codes.append("pure_random")
    ...
    event = Event(
        ...
        selection_method=(
            "momentum_weighted"
            if max_bonus > 0 and bandwidth_applied
            else "momentum"
            if max_bonus > 0
            else "bandwidth"
            if bandwidth_applied
