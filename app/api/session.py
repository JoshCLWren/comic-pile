...
async def _bulk_unread_counts(threads: list[Thread], db: AsyncSession) -> dict[int, int]:
    """Bulk-load unread issue counts for all migrated threads in one query."""
...
async def _bulk_next_issue_numbers(threads: list[Thread], db: AsyncSession) -> dict[int, str]:
    """Bulk-load next unread issue numbers for migrated threads."""
...
async def build_narrative_summary(session_id: int, db: AsyncSession) -> dict[str, list[str]]:
    ...
    summary = {"read": [], "skipped": [], "completed": []}
    ...
    while retries < max_retries:
        try:
            ...
            snapshot_count_result = await db.execute(
                select(func.count()).select_from(Snapshot).where(Snapshot.session_id == active_session_id)
            )
...
async def list_sessions(
    ...
    ladder_paths = {
        sid: " → ".join(str(d) for d in [sessions_by_id[sid].start_die, *projection.die_path_by_session[sid]])
        for sid in session_ids
    }
    current_die: dict[int, int] = {}
    for sid in session_ids:
        session = sessions_by_id[sid]
        current_die[sid] = session.manual_die or projection.latest_die_by_session.get(sid, session.start_die)
    thread_ids = {
    ...
    if thread_ids:
        ...
        for sid in session_ids:
            ...
            if roll_event is not None and roll_event.selected_thread_id is not None:
                ...
                if thread:
                    issues_remaining = unread_counts.get(thread.id, 0) if thread.uses_issue_tracking() else thread.issues_remaining
                    issue_id = thread.next_unread_issue_id if thread.uses_issue_tracking() else None
                    issue_number = issue_numbers.get(issue_id) if issue_id is not None else None
...
async def get_session_details(
    ...
    for event in events:
        thread_id = event.selected_thread_id if event.type == "roll" else event.thread_id
        thread_title = threads_dict[thread_id].title if thread_id in threads_dict else None
...
async def restore_session_start(
    ...
    while retries < max_retries:
        try:
            ...
            current_threads_result = await db.execute(select(Thread).where(Thread.user_id == current_user.id))
