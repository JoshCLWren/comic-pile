...
async def test_invalid_payloads_rejected(
    ...
    response = await auth_client.post(f"/api/sessions/{session.id}/mode", json=payload)
...
async def test_unknown_session_returns_404(auth_client: AsyncClient) -> None:
    ...
    get_response = await auth_client.get("/api/sessions/999999/mode")
    assert get_response.status_code == 404

    post_response = await auth_client.post(
        "/api/sessions/999999/mode",
...
@pytest.mark.asyncio
async def test_mode_endpoint_mounted_under_v1_prefix(
    auth_client: AsyncClient,
    async_db: AsyncSession,
) -> None:
    """The canonical /api/v1 prefix serves the same contract."""
    user = await get_or_create_user_async(async_db)
    session = await _create_session(async_db, user.id)
