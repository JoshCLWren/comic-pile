...
def test_selected_executor_metadata_reaches_worker_and_telemetry() -> None:
    ...
    heartbeat_model = (
        "${{ steps.route.outputs.model || steps.executor.outputs.model "
        "|| steps.lane.outputs.model }}"
    )
    session_model = (
        "${{ steps.smoke.outputs.model || steps.route.outputs.model "
        "|| steps.executor.outputs.model || steps.lane.outputs.model }}"
    )
    session_runtime = (
        "${{ steps.smoke.outputs.runtime_model || steps.route.outputs.runtime_model "
    ...
    assert f"MODEL: {heartbeat_model}" in workflow
    assert f"FACTORY_MODEL: {session_model}" in workflow
    assert f"FACTORY_RUNTIME_MODEL: {session_runtime}" in workflow
