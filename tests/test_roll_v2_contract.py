...
from app.schemas.roll import RollRecoveryInfo, RollRecoveryPrerequisite, RollRecoveryChainNode
from app.schemas.session import SessionMode, ActiveThreadInfo, SessionBandwidthState
...
class TestRollV2BootstrapResponse:
    ...
    def test_rollable_item_structure(self):
        ...
        identity = RollableIdentity(
            ...
            series_mapping_state="unavailable"
        )
...
class TestV2BootstrapEndpointContract:
    ...
    def test_no_unversioned_alias(self):
        ...
        assert RollV2BootstrapResponse != RollRecoveryInfo  # Different from v1
    ...
    def test_rollable_issue_required_non_null(self):
        ...
        assert issue_field.default is None  # No default means required
        assert issue_field.annotation is not RollableIssue | None  # Not nullable means required
...
class TestRateResponseContract:
    ...
    def test_adds_roll_reconciliation(self):
        ...
        # Verify RollReconciliation has last_read
        assert hasattr(RollReconciliation, 'last_read')
        assert RollReconciliation.model_fields['last_read'].annotation == RollLastRead | None


# Integration test would require database setup and actual API calls
# These are focused contract tests that verify the schema structure meets requirements