...
@pytest.mark.asyncio
async def test_roll_respects_collection_filter(
    auth_client: AsyncClient, async_db: AsyncSession
) -> None:
    """Roll API should respect collection_id filter when provided (issue #284)."""
    from app.models import Collection, Thread
    from tests.conftest import get_or_create_user_async

    user = await get_or_create_user_async(async_db)

    marvel_collection = Collection(name="Marvel", user_id=user.id, is_default=True, position=1)
    dc_collection = Collection(name="DC", user_id=user.id, is_default=False, position=2)
    async_db.add(marvel_collection)
    async_db.add(dc_collection)
    await async_db.commit()
    await async_db.refresh(marvel_collection)
    await async_db.refresh(dc_collection)

    marvel_thread = Thread(
        title="Spider-Man",
        format="Comic",
        issues_remaining=10,
        queue_position=1,
        ...
        is_blocked=False,
        user_id=user.id,
        collection_id=marvel_collection.id,
    )
    dc_thread1 = Thread(
        title="Batman",
        format="Comic",
        ...
        is_blocked=False,
        user_id=user.id,
        collection_id=dc_collection.id,
    )
    dc_thread2 = Thread(
        title="Superman",
        format="Comic",
    ...
    async_db.add(marvel_thread)
    async_db.add(dc_thread1)
    async_db.add(dc_thread2)
    async_db.add(dc_thread3)
    await async_db.commit()
    await async_db.refresh(marvel_thread)
    await async_db.refresh(dc_thread1)
    await async_db.refresh(dc_thread2)
    await async_db.refresh(dc_thread3)

    dc_thread_ids = {dc_thread1.id, dc_thread2.id, dc_thread3.id}

    for i in range(20):
        response = await auth_client.post(
            "/api/roll/", json={"collection_id": marvel_collection.id}
        )
        assert response.status_code == 200
