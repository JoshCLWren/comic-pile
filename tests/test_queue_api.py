...
async def test_move_to_position(client, db, sample_data):
    """PUT /threads/{id}/position/ works."""
    thread_id = 3
    new_position = 1

    response = await client.put(
        f"/threads/{thread_id}/position/", json={"new_position": new_position}
    )
    ...
    thread2 = db.get(Thread, 1)
    thread4 = db.get(Thread, 4)
...
async def test_move_to_front(client, db, sample_data):
    """PUT /threads/{id}/front/ moves to position 1."""
    thread_id = 5

    response = await client.put(f"/threads/{thread_id}/front/")
    assert response.status_code == 200
    ...
    thread1 = db.get(Thread, 1)
    assert thread1.queue_position == 2


@pytest.mark.asyncio
async def test_move_to_back(client, db, sample_data):
    """PUT /threads/{id}/back/ moves to last position."""
    thread_id = 1

    response = await client.put(f"/threads/{thread_id}/back/")
    assert response.status_code == 200
...
async def test_queue_order_preserved(client, db, sample_data):
    ...
    thread3 = db.get(Thread, 3)
    thread4 = db.get(Thread, 4)
    thread5 = db.get(Thread, 5)
...
async def test_move_to_position_invalid(client, db, sample_data):
    ...
    response = await client.put(f"/threads/{thread_id}/position/", json={"new_position": 999})
    assert response.status_code == 200
...
async def test_move_to_position_zero(client, db, sample_data):
    ...
    response = await client.put(f"/threads/{thread_id}/position/", json={"new_position": 0})
    assert response.status_code == 200
...
async def test_move_nonexistent_thread(client, db, sample_data):
    ...
    response = await client.put("/threads/999/front/")
