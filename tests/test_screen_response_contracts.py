...
def test_queue_item_records_serialized_byte_reduction() -> None:
    ...
    detail = ThreadResponse(
        ...
        total_issues=12,
        reading_progress=(
            "Issue 1 of 12 read; continuing with issue 2 after Mister Miracle #1."
        ),
...
def test_session_history_item_contract_is_exact_and_measurably_narrower() -> None:
    ...
    assert len(full_fields) == 23
    assert len(history_fields) == 16
    assert (len(full_fields) - len(history_fields)) / len(full_fields) == 7 / 23
...
def test_current_session_contract_is_exact_and_named() -> None:
    """The current-session screen exposes exactly the named 23-field contract."""
    assert set(SessionResponse.model_fields) == CURRENT_SESSION_FIELDS
    assert len(SessionResponse.model_fields) == 23
...
def test_no_collection_specific_contracts_remain_in_openapi() -> None:
    ...
    schema = app.openapi()
    collection_paths = [path for path in schema["paths"] if "collection" in path.lower()]
    collection_schemas = [
        name
