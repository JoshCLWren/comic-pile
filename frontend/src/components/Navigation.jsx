  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 nav-container z-40" role="navigation" aria-label="Main navigation">
      <div className="flex justify-around items-center h-20 px-2 max-w-lg md:max-w-2xl lg:max-w-4xl xl:max-w-5xl mx-auto">
        ...
        <Link to="/settings" className={`nav-item flex flex-col items-center justify-center flex-1 h-full transition-all duration-200 focus:outline-none ${isActive('/settings') ? 'active' : 'hover:bg-white/5'}`} aria-label="Settings page">
          <span className="text-2xl mb-1" aria-hidden="true">⚙️</span>
          <span className="text-[10px] uppercase tracking-widest font-bold nav-label">Settings</span>
        </Link>
      </div>
    </nav>
  )
