...
function createTextureAtlas(maxNumber: number, renderConfig: RenderConfig): AtlasInfo {
  const {
    tileSize,
    uvInset,
    fontScale,
...
function getUVForNumber(number: number, cols: number, rows: number, uvInset = 0): UV {
...
function createD4Geometry(atlasInfo: AtlasInfo): THREE.BufferGeometry {
...
function createD6Geometry(atlasInfo: AtlasInfo): THREE.BufferGeometry {
...
function createD8Geometry(atlasInfo: AtlasInfo): THREE.BufferGeometry {
  const { cols, rows, uvInset, triangleUvRadius } = atlasInfo;
  const verts: number[] = [];
  const uvs: number[] = [];
  const inds: number[] = [];
...
function createD10Geometry(atlasInfo: AtlasInfo): THREE.BufferGeometry {
  const {
    cols,
    rows,
    uvInset,
  ...
  function calculateFaceCenter(quad: number[][]): number[] {
    const [a, b, c, d] = quad;
  ...
  function calculateFaceNormal(a: number[], b: number[], c: number[]): number[] {
  ...
  function ensureOutwardQuad(quad: number[][]): number[][] {
  ...
  function calculateFaceBasis(normal: number[]): { uAxis: number[]; vAxis: number[] } {
    let arb = [1, 0, 0];
  ...
  function projectVertexToPlane(vertex: number[], center: number[], uAxis: number[], vAxis: number[]): { u: number; v: number } {
  ...
  function calculateUVBounds(projA: { u: number; v: number }, projB: { u: number; v: number }, projC: { u: number; v: number }, projD: { u: number; v: number }) {
  ...
  function normalizeToBounds(u: number, v: number, bounds: { uMin: number; uMax: number; vMin: number; vMax: number }) {
  ...
  function mapToTextureTile(uNorm: number, vNorm: number, tileUv: UV, tilePadding: number) {
    return {
...
function createD12Geometry(atlasInfo: AtlasInfo): THREE.BufferGeometry {
...
function createD20Geometry(atlasInfo: AtlasInfo): THREE.BufferGeometry {
  const { cols, rows, uvInset, triangleUvRadius } = atlasInfo;
  const verts: number[] = [];
  const uvs: number[] = [];
  const inds: number[] = [];
...
function buildGeometry(sides: number, atlasInfo: AtlasInfo): THREE.BufferGeometry {
  switch (sides) {
    case 4:
...
function buildNumberNormals(geometry: THREE.BufferGeometry, cols: number, rows: number): Map<number, THREE.Vector3> {
...
function getFaceRotation(value: number, normalMap: Map<number, THREE.Vector3> | null): Rotation {
...
export default function Dice3D({
  ...
  const numberNormalsRef = useRef<Map<number, THREE.Vector3> | null>(null);
