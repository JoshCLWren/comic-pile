...
type WindowWithShortTimeoutFlag = Window & { __COMIC_PILE_SHORT_TIMEOUT__?: boolean };
...
async function installShortAxiosTimeout(page: import('@playwright/test').Page): Promise<void> {
  await page.addInitScript((timeoutMs: number) => {
    const globalScope = window as WindowWithShortTimeoutFlag;
    ...
    Object.defineProperty(XMLHttpRequest.prototype, 'timeout', {
      ...
      set(value: number) {
        const clampActive =
          (window as WindowWithShortTimeoutFlag).__COMIC_PILE_SHORT_TIMEOUT__ === true;
