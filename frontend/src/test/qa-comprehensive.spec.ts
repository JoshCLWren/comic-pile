...
test('1. Auth - Navigate to home and verify authentication', async ({ authenticatedPage }) => {
    // Navigate to home
    ...
    await expect(authenticatedPage.locator('[data-roll-pool]')).toBeVisible({ timeout: 10000 });
  });

  test('2. Roll page - Verify page loads and elements are present', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/', { waitUntil: 'domcontentloaded' });
  ...
  // Check header indicators
    const headerElements = authenticatedPage.locator('[data-roll-pool] header, [data-roll-pool] .header');
    const headerCount = await headerElements.count();

    if (headerCount > 0) {
      // Check for labels and tooltips (hover over indicators)
      const indicators = authenticatedPage.locator('[data-roll-pool] [aria-label], [data-roll-pool] [title]');
...
test('3. Queue page - Verify thread list and search/filter', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/queue', { waitUntil: 'domcontentloaded' });

    // Verify queue page loads
    await expect(authenticatedPage.locator('h1:has-text("Queue"), h2:has-text("Queue")').first()).toBeVisible({ timeout: 10000 });

    // Check search/filter box
    const searchBox = authenticatedPage.locator('input[placeholder*="search" i], input[placeholder*="filter" i], input[type="search"]');
    ...
    const threadCards = authenticatedPage.locator('[data-testid="queue-thread-item"], .thread-card, .thread-item');
...
test('4. Session page - Click into thread and verify issues', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/queue', { waitUntil: 'domcontentloaded' });
    ...
    if (threadCount > 0) {
      ...
      const issueList = authenticatedPage.locator('.issue-list, [data-testid="issue-list"]');
      await expect(issueList.first()).toBeVisible({ timeout: 10000 });

      // Check issue cards
      const issueCards = authenticatedPage.locator('.issue-item, [data-issue-id]');
      ...
      const positionSlider = authenticatedPage.locator('input[type="range"], .position-slider, [data-position-slider]');
...
test('5. History page - Verify reading history', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/history', { waitUntil: 'domcontentloaded' });

    // Verify history page loads
    await expect(authenticatedPage.locator('h1:has-text("History"), h2:has-text("History")').first()).toBeVisible({ timeout: 10000 });

    // Check for sessions list
    const sessionsList = authenticatedPage.locator('#sessions-list, .sessions-list, [data-sessions]');
