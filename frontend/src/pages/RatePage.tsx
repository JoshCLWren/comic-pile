...
export default function RatePage(): React.JSX.Element {
  const navigate = useNavigate()
  const location = useLocation()
  const rollResponse = location.state?.rollResponse as RollResponse | undefined

  const [rating, setRating] = useState<number>(4.0)
  const [predictedDie, setPredictedDie] = useState<number>(6)
  const [currentDie, setCurrentDie] = useState<number>(6)
  const [previewSides, setPreviewSides] = useState<number>(6)
  const [rolledValue, setRolledValue] = useState<number>(1)
  const [errorMessage, setErrorMessage] = useState<string>('')
  const [showCompleteModal, setShowCompleteModal] = useState<boolean>(false)
  const [showAddIssuesInput, setShowAddIssuesInput] = useState<boolean>(false)
  const [additionalIssues, setAdditionalIssues] = useState<number>(1)
  ...
  async function handleSubmitRating(finishSession = false): Promise<void> {
    ...
    try {
      ...
      setErrorMessage(err.response?.data?.detail || 'Failed to save rating');
    }
  }

  async function handleCompleteThread(): Promise<void> {
    setShowCompleteModal(false);
    if (pendingRating !== null && pendingRating >= 4.0) {
      createExplosion();
    }
    try {
      await rateMutation.mutate({
  ...
  async function handleAddMoreIssues(): Promise<void> {
    ...
    try {
      ...
      const err = error as ApiErrorResponse
      setErrorMessage(err.response?.data?.detail || 'Failed to add issues. Please try again.');
      return;
    }
  ...
  async function handleSnooze(): Promise<void> {
    try {
      ...
      const err = error as ApiErrorResponse
      setErrorMessage(err.response?.data?.detail || 'Failed to snooze thread');
    }
  }

  async function navigateAfterRating(): Promise<void> {
    try {
        ...
        <div className="space-y-12 relative z-10">
          ...
          <div className="text-center space-y-6">
            ...
            <input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateUI(e.target.value)}
            />
          </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAdditionalIssues(Math.max(1, parseInt(e.target.value) || 1))}
