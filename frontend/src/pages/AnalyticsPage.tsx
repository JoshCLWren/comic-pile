...
import { useAnalytics, RecentSession, TopRatedThread } from '../hooks/useAnalytics'
...
export default function AnalyticsPage(): React.JSX.Element {
  ...
  // Type guard to ensure metrics is defined
  if (!metrics) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-red-400">No analytics data available</div>
      </div>
    )
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="glass-card p-4">
          ...
          {metrics.recent_sessions && metrics.recent_sessions.length > 0 ? (
            <div className="space-y-2">
              {metrics.recent_sessions.slice(0, 5).map((session: RecentSession) => (
        ...
        <div className="glass-card p-4">
          ...
          {metrics.event_stats && Object.keys(metrics.event_stats).length > 0 ? (
            <div className="space-y-2">
              {Object.entries(metrics.event_stats).map(([eventType, count]: [string, number]) => (
      ...
      <div className="glass-card p-4">
        ...
        {metrics.top_rated_threads && metrics.top_rated_threads.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {metrics.top_rated_threads.map((thread: TopRatedThread) => (
