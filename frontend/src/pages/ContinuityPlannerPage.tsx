...
export default function ContinuityPlannerPage() {
  ...
  const isDirty =
    name !== savedName || JSON.stringify(nodes) !== JSON.stringify(savedNodes)

  const hydrateLabels = useCallback(
    async (rawNodes: ContinuityPlanNode[], loadedGroups: DependencyGroup[]) => {
      const groupNames = new Map(loadedGroups.map((group) => [group.id, group.name]))
      return Promise.all(
        rawNodes.map(async (node): Promise<PlannerNode> => {
          if (node.node_type === 'crossover') {
            return {
              ...node,
              label: groupNames.get(node.ref_id) ?? 'Unavailable crossover',
            }
          }
          try {
  ...
  useEffect(() => {
    ...
    void Promise.all([fetchAllThreads(), dependencyGroupsApi.list()])
      ...
      .catch((error) =>
        active && setLoadError(errorMessage(error, 'Unable to load the continuity planner.')),
      )
  ...
  return (
    <section className="space-y-5" aria-labelledby="planner-heading">
      <header>
        ...
        <h1 id="planner-heading" className="mt-1 text-3xl font-black text-stone-100">
          Sequential planner
        </h1>
        <p className="mt-2 text-sm text-stone-400">
          Arrange issues and crossovers in one explicit reading lane. Saving
          creates only the continuity rules you chose.
        </p>
      </header>
      ...
      {!planId && lastPlanId && (
        <button
          type="button"
          onClick={() => navigate(`/continuity-plans/${lastPlanId}`)}
          className="min-h-11 rounded-xl border border-amber-700 px-4 font-bold text-amber-200"
        >
      ...
      <label className="block text-xs font-bold uppercase tracking-widest text-stone-500">
        ...
        <input
          ...
          maxLength={200}
          className="mt-1 w-full rounded-xl border border-stone-700 bg-stone-950 px-3 py-3 text-stone-100"
        />
      </label>
      ...
      <div className="grid gap-4 rounded-2xl border border-stone-800 bg-stone-900/50 p-4 md:grid-cols-2">
        <form onSubmit={addIssue} className="space-y-3" aria-label="Add an issue">
          ...
          <ContinuityThreadSelector
            ...
            onChange={(thread) => void selectThread(thread)}
            label="Comic series"
          />
          <ContinuityIssueSelector
            issues={issues}
          ...
          <button
            ...
            className="min-h-11 w-full rounded-xl bg-amber-500 px-4 font-black text-stone-950 disabled:opacity-50"
          >
            Add issue
          </button>
        </form>
      ...
      <section aria-labelledby="lane-heading">
        <div className="flex items-end justify-between">
          <div>
            <h2 id="lane-heading" className="text-xl font-black text-stone-100">
              Reading order
            </h2>
          ...
          {statusText && (
            <p
              role="status"
              className={isDirty ? 'text-amber-300' : 'text-emerald-300'}
            >
              {statusText}
            </p>
          )}
        ...
        {nodes.length === 0 ? (
          <p className="mt-3 rounded-2xl border border-dashed border-stone-700 p-6 text-center text-stone-500">
            Add an issue or crossover to begin.
          </p>
        ) : (
          <ol className="mt-3 grid gap-2">
            {nodes.map((node, index) => (
              <li
                key={node.id}
                className="flex items-center gap-3 rounded-2xl border border-stone-700 bg-stone-900 p-3"
              >
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-stone-800 font-black text-amber-300">
                  {index + 1}
                </span>
                ...
                <div className="flex gap-1">
                  <button
                    ...
                    className="min-h-11 min-w-11 rounded-lg border border-stone-700 disabled:opacity-30"
                  >
                    ↑
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="min-h-11 rounded-lg border border-red-900 px-3 text-red-300"
                  >
                    Remove
                  </button>
                </div>
      ...
      {saveError && (
        <p
          role="alert"
          className="rounded-xl border border-red-800 bg-red-950/30 p-3 text-red-200"
        >
          {saveError}
        </p>
      )}
      ...
      <div className="sticky bottom-16 flex gap-3 rounded-2xl border border-stone-800 bg-stone-950/95 p-3 backdrop-blur md:bottom-24">
        <button
          ...
          className="min-h-11 flex-1 rounded-xl border border-stone-700 font-bold disabled:opacity-40"
        >
          Cancel changes
        </button>
        <button
          ...
          onClick={() => void save()}
          disabled={!isDirty || isSaving}
          className="min-h-11 flex-1 rounded-xl bg-amber-500 font-black text-stone-950 disabled:opacity-40"
        >
