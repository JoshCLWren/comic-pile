...
export default function RollPage(): React.JSX.Element {
  const [isRolling, setIsRolling] = useState<boolean>(false)
  const [rolledResult, setRolledResult] = useState<number | null>(null)
  const [rolledOffset, setRolledOffset] = useState<number | null>(null)
  const [selectedThreadId, setSelectedThreadId] = useState<number | null>(null)
  const [currentDie, setCurrentDie] = useState<number>(6)
  const [diceState, setDiceState] = useState<DiceState>('idle')
  const [staleThread, setStaleThread] = useState<StaleThread | null>(null)
  const [isOverrideOpen, setIsOverrideOpen] = useState<boolean>(false)
  const [overrideThreadId, setOverrideThreadId] = useState<string>('')
  const [snoozedExpanded, setSnoozedExpanded] = useState<boolean>(false)
  ...
  async function handleUnsnooze(threadId: number): Promise<void> {
    try {
  ...
  async function handleReadStale(): Promise<void> {
  ...
  function setDiceStateValue(state: DiceState): void {
    setDiceState(state)
  }

  async function handleSetDie(die: number): Promise<void> {
    setCurrentDie(die)
    await setDieMutation.mutate(die)
  }

  async function handleClearManualDie(): Promise<void> {
    ...
    rollIntervalRef.current = setInterval(() => {
      ...
      if (currentRollCount >= maxRolls) {
        ...
        rollTimeoutRef.current = setTimeout(async () => {
           ...
           try {
             const response: RollResponse = await rollMutation.mutate()
             if (response?.result) {
  ...
  function handleRollComplete(): void {
    setDiceStateValue('rolled')
  }

  function handleOverrideSubmit(event: React.FormEvent<HTMLFormElement>): void {
    event.preventDefault()
          ...
          {!isRolling && rolledResult && (
            <div className="text-center shrink-0">
              <div className="roll-value flex items-center justify-center gap-1">
                ...
                {rolledOffset !== null && rolledOffset > 0 && (
            ...
            <div className="flex-1 overflow-y-auto mt-2 space-y-2 scrollbar-thin">
              {pool.length === 0 ? (
                ...
                pool.map((thread, index) => {
                  const isSelected = selectedThreadId && parseInt(String(selectedThreadId), 10) === thread.id
                  return (
                    <div
                      key={thread.id}
        ...
        <form className="space-y-4" onSubmit={handleOverrideSubmit}>
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(event: React.ChangeEvent<HTMLSelectElement>) => setOverrideThreadId(event.target.value)}
