...
const crossover = {
  ...
  memberships: [
    { id: 1, issue_id: 31, thread_id: null, sequence_order: 1, series_title: 'Nova', issue_number: '2' },
...
describe('CrossoversPage membership editing', () => {
  ...
  it('renders a readable fallback when member metadata cannot be resolved', async () => {
    api.list.mockResolvedValue([{
      ...crossover,
      memberships: [{ id: 9, issue_id: 99, thread_id: null, sequence_order: 1, series_title: null, issue_number: null }],
    }])
    renderPage()
  ...
  it('adds a whole thread from the shared human-facing selector', async () => {
    api.addMember.mockResolvedValue({ id: 3, issue_id: null, thread_id: 44, sequence_order: 3, series_title: 'Uncanny X-Men', issue_number: null })
    renderPage()
    fireEvent.click(await screen.findByRole('button', { name: /Annihilation.*2 members/ }))
  ...
  it('preserves unrelated crossovers while adding and removing memberships', async () => {
    const unrelated = {
      ...
      created_at: '2026-08-06T00:00:00Z',
      memberships: [{ id: 8, issue_id: 80, thread_id: null, sequence_order: 1, series_title: 'Mighty Avengers', issue_number: '12' }],
    }
