...
it('loads thread details and stale list', async () => {
  const { result: threadResult } = renderQueryHook(() => useThread(2))
  const { result: staleResult } = renderQueryHook(() => useStaleThreads(7))
...
it('creates, updates, deletes, and reactivates threads', async () => {
  const { result: createResult } = renderQueryHook(() => useCreateThread())
  await act(async () => {
    await createResult.current.mutate({ title: 'New', format: 'Comics', issues_remaining: 5 })
  })

  const { result: updateResult } = renderQueryHook(() => useUpdateThread())
  await act(async () => {
    await updateResult.current.mutate({ id: 7, data: { title: 'Updated' } })
  })

  const { result: deleteResult } = renderQueryHook(() => useDeleteThread())
  await act(async () => {
    await deleteResult.current.mutate(7)
  })

  const { result: reactivateResult } = renderQueryHook(() => useReactivateThread())
  await act(async () => {
...
it('handles non-Error mutation failures and stale refetch failures', async () => {
  ...
  const { result } = renderQueryHook(() => useStaleThreads(3))
...
it('ignores late detail and stale responses after unmount', async () => {
  ...
  const detail = renderQueryHook(() => useThread(10))
  const stale = renderQueryHook(() => useStaleThreads(10))
  detail.unmount()
  stale.unmount()
  await act(async () => {
...
it('ignores late detail and stale failures after unmount', async () => {
  ...
  const detail = renderQueryHook(() => useThread(11))
  const stale = renderQueryHook(() => useStaleThreads(11))
  detail.unmount()
  stale.unmount()
  await act(async () => {
...
it('uses response details and ignores late hook results after unmount', async () => {
  ...
  mockedThreadsApi.create.mockRejectedValueOnce(axiosError)
  const { result } = renderQueryHook(() => useCreateThread())
  ...
  const pending = renderQueryHook(() => useThread(44))
  pending.unmount()
...
it('ignores late stale-thread results after unmount', async () => {
  ...
  const pending = renderQueryHook(() => useStaleThreads(14))
  pending.unmount()
