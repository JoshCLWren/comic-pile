...
it('renders tooltips for snoozed offset active text', async () => {
  ...
  // Check for snoozed tooltip target - use getAllByText and pick the one with cursor-help
  const snoozedElements = screen.getAllByText(/snoozed/i)
  const snoozedTooltipTarget = snoozedElements.find(el => el.classList.contains('cursor-help'))
  expect(snoozedTooltipTarget).toBeDefined()
  ...
  // Check that offset and active labels are rendered (they have tooltips but no cursor-help class)
  // They are wrapped in Tooltip components but don't have the dashed underline style
