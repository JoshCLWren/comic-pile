...
describe('ContinuityPlannerPage', () => {
  ...
  it('restores saved order when unsaved changes are canceled', async () => {
    ...
    expect(
      await screen.findByRole('button', { name: /Remove.*Fourth World/ }),
    ).toBeVisible()
    ...
    expect(
      screen.getByRole('button', { name: /Remove.*Fourth World/ }),
    ).toBeVisible()
    expect(mocks.update).not.toHaveBeenCalled()
  })

  it('reorders nodes and normalizes positions before saving', async () => {
    mocks.get.mockResolvedValue({
      id: 12,
      user_id: 1,
      name: 'Plan with nodes',
      ordering_mode: 'strict_sequential',
      nodes: [
        {
          id: 'issue-40',
    ...
    const moveLaterButtons = screen.getAllByRole('button', { name: /later$/i })
    await user.click(moveLaterButtons[0])
    await user.click(screen.getByRole('button', { name: 'Save plan' }))

    await waitFor(() => expect(mocks.update).toHaveBeenCalledOnce())
    const updatedPayload = mocks.update.mock.calls[0][1]

    expect(updatedPayload.name).toBe('Plan with nodes')
    expect(
      updatedPayload.nodes.map((node: { ref_id: number }) => node.ref_id),
    ).toEqual([8, 40])
    expect(
      updatedPayload.nodes.map((node: { position: number }) => node.position),
    ).toEqual([0, 1])
  })
})
