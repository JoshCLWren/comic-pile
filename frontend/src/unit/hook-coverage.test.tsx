...
describe('mutation hook success and failure paths', () => {
  it('runs queue and rating mutations', async () => {
    const position = renderQueryHook(() => useMoveToPosition()); await act(async () => await position.result.current.mutate({ id: 1, position: 2 }))
    const front = renderQueryHook(() => useMoveToFront()); await act(async () => await front.result.current.mutate(1))
    const back = renderQueryHook(() => useMoveToBack()); await act(async () => await back.result.current.mutate(1))
    const shuffle = renderQueryHook(() => useShuffleQueue()); await act(async () => await shuffle.result.current.mutate())
    expect(front.result.current.isError).toBe(false)
    const rate = renderQueryHook(() => useRate())
  ...
  it('sets error state and rethrows mutation failures', async () => {
    ...
    const { result } = renderQueryHook(() => useMoveToFront())
    await act(async () => {
    ...
    const back = renderQueryHook(() => useMoveToBack())
    ...
    const position = renderQueryHook(() => useMoveToPosition())
    ...
    const shuffle = renderQueryHook(() => useShuffleQueue())
  ...
  it('runs all roll mutations', async () => {
    const roll = renderQueryHook(() => useRoll()); await act(async () => await roll.result.current.mutate())
    const override = renderQueryHook(() => useOverrideRoll()); await act(async () => await override.result.current.mutate({ thread_id: 1 }))
    const dismiss = renderQueryHook(() => useDismissPending()); await act(async () => await dismiss.result.current.mutate())
    const setDie = renderQueryHook(() => useSetDie()); await act(async () => await setDie.result.current.mutate(6))
    const clearDie = renderQueryHook(() => useClearManualDie()); await act(async () => await clearDie.result.current.mutate())
    const reroll = renderQueryHook(() => useReroll()); await act(async () => await reroll.result.current.mutate())
    expect(roll.result.current.isError).toBe(false)
    api.rollApi.reroll.mockRejectedValueOnce(new Error('fail'))
    const failed = renderQueryHook(() => useReroll())
    await act(async () => {
    ...
    const failedRoll = renderQueryHook(() => useRoll())
    ...
    const failedOverride = renderQueryHook(() => useOverrideRoll())
    ...
    const failedDismiss = renderQueryHook(() => useDismissPending())
    ...
    const failedDie = renderQueryHook(() => useSetDie())
    ...
    const failedClear = renderQueryHook(() => useClearManualDie())
...
describe('data hooks', () => {
  it('loads analytics and snapshots', async () => {
    ...
    const analytics = renderQueryHook(() => useAnalytics())
    ...
    const stringAnalytics = renderQueryHook(() => useAnalytics())
    ...
    const failedSnapshots = renderQueryHook(() => useSnapshots(2))
  ...
  it('loads sessions through pagination and details', async () => {
    ...
    const sessions = renderQueryHook(() => useSessions())
    ...
    const details = renderQueryHook(() => useSessionDetails(1))
    await waitFor(() => expect(details.result.current.data).toEqual({ session_id: 1 }))
    const noDetails = renderQueryHook(() => useSessionDetails(null))
  ...
  it('handles loadMore failure with Error and non-Error types', async () => {
    ...
    const sessions2 = renderQueryHook(() => useSessions())
  ...
  it('loads current session, handles notifications, and restores snapshots', async () => {
    ...
    const current = renderQueryHook(() => useSession())
    ...
    const snapshots = renderQueryHook(() => useSessionSnapshots(1))
    await waitFor(() => expect(snapshots.result.current.data).toEqual({ snapshots: [] }))
    api.sessionApi.restoreSessionStart.mockResolvedValue({ ok: true })
    const restore = renderQueryHook(() => useRestoreSessionStart())
    ...
    const undo = renderQueryHook(() => useUndo())
    ...
    const failedUndo = renderQueryHook(() => useUndo())
  ...
  it('covers session error and empty-id branches', async () => {
    ...
    const sessions = renderQueryHook(() => useSessions(null as never))
    await waitFor(() => expect(sessions.result.current.isError).toBe(true))
    api.sessionApi.list.mockRejectedValueOnce('string list failed')
    const sessionsStr = renderQueryHook(() => useSessions())
    ...
    const details = renderQueryHook(() => useSessionDetails(9))
    ...
    const snapshots = renderQueryHook(() => useSessionSnapshots(9))
  ...
  it('covers storage fallbacks, unchanged sessions, empty snapshots, and Axios restore errors', async () => {
    ...
    const emptySnapshots = renderQueryHook(() => useSessionSnapshots(null))
  ...
  it('ignores snapshots that resolve after unmount', async () => {
    ...
    const snapshots = renderQueryHook(() => useSnapshots(44))
    snapshots.unmount()
  ...
  it('covers roll bootstrap success, error, and refetch paths', async () => {
    ...
    const hook = renderQueryHook(() => useRollBootstrap())
    ...
    const failedHook = renderQueryHook(() => useRollBootstrap())
    await waitFor(() => {
    ...
    const stringHook = renderQueryHook(() => useRollBootstrap())
    await waitFor(() => {
  ...
  it('covers stale thread error path', async () => {
    ...
    const stale = renderQueryHook(() => useStaleThreads())
