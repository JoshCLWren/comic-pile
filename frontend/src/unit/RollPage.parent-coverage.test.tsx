...
// SAFETY: ThreadPool mock casts Record<string, unknown> props to the exact callback signatures RollPage supplies, so invoking them with the declared arg lists is type-safe in the rendered harness.
...
vi.mock('../pages/RollPage/components/RatingView', () => ({ RatingView: (props: Record<string, unknown>) => {
  // SAFETY: RollPage always supplies activeRatingThread with only title/issue_number consumed by this mock render, so the object-shaped cast preserves the read shape without null-throwing accesses.
