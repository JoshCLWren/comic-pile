...
function ControlLabel({ label }: ControlLabelProps): React.ReactElement {
...
function buildValueList(sides: number): number[] {
...
export default function DicePlayground(): React.ReactElement {
  ...
  const [jsonDraft, setJsonDraft] = useState<string>(() => JSON.stringify(DEFAULT_DICE_RENDER_CONFIG, null, 2))
  ...
  function syncDraft(nextConfig: DiceRenderConfig): void {
    setRenderConfig(nextConfig)
    setJsonDraft(JSON.stringify(nextConfig, null, 2))
  }

  function handleSidesClick(nextSides: number): void {
    setSides(nextSides)
    setValue(1)
  }

  function updateGlobal(key: keyof GlobalConfig, nextValue: string | number | boolean): void {
    const nextConfig: DiceRenderConfig = {
      ...renderConfig,
      global: {
        ...renderConfig.global,
        [key]: nextValue,
  ...
  function updateSideOverride(key: keyof SideConfig, nextValue: string | number | boolean): void {
  ...
  function clearSideOverride(): void {
    const sideKey = String(sides)
  ...
  function tryApplyJson(rawJson: string): void {
    try {
      const parsed = JSON.parse(rawJson) as Partial<DiceRenderConfig>
      const merged: DiceRenderConfig = {
  ...
  const sideOverride: SideConfig = renderConfig.perSides?.[String(sides)] ?? {}
        ...
        <section className="space-y-4 rounded-2xl border border-cyan-500/30 bg-slate-950/60 p-4 xl:sticky xl:top-4">
          ...
          >
            <LazyDice3D
              ...
              lockMotion={isTuning}
              color={0xffffff}
              renderConfig={renderConfig as unknown as Record<string, unknown>}
            />
          </div>
        ...
        <section className="space-y-6 xl:max-h-[calc(100vh-2rem)] xl:overflow-y-auto xl:pr-2">
          ...
          <section className="space-y-3 rounded-2xl border border-slate-700 bg-slate-950/60 p-4">
            ...
            <textarea
              ...
              value={jsonDraft}
              onChange={(event: React.ChangeEvent<HTMLTextAreaElement>) => setJsonDraft(event.target.value)}
              onFocus={beginTuning}
              onBlur={endTuning}
