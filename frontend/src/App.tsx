...
function AuthenticatedLayout({ children, onBugReportSubmit, wide = false }: { children: ReactNode; onBugReportSubmit: BugReportSubmit; wide?: boolean }) {
  const mainMaxWidth = wide
    ? 'max-w-(1536px)'
    : 'max-w-lg md:max-w-2xl lg:max-w-4xl xl:max-w-5xl'
  return <div className="flex min-h-screen" data-app-shell-ready><main className={`flex-1 container mx-auto px-3 md:px-4 py-4 md:py-6 ${mainMaxWidth} pb-28`}>{children}</main><Navigation onBugReportSubmit={onBugReportSubmit} wide={wide} /></div>
}
