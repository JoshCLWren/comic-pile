...
# Stage 4 made these the hard-failure states in the mechanical merge authority.
# The work controller mirrors that exact classification without importing a
# controller or changing the stage-4 authority.
REQUIRED_CHECK_FAILURE_STATES = frozenset(
    {
        "CANCELLED",
        "ERROR",
        "FAILURE",
...
NO_DIFF_ATTEMPT_RE = re.compile(
    r"<!--\s*comic-pile-factory-claim-released-v3:issue-(?P<issue>\d+):"
    r"(?P<worker>[^:>\s]+):(?P<epoch>\d{10}):no-persisted-change-handoff\s*-->"
)
...
FACTORY_NO_DIFF_RETRY_RESET_SECONDS = env_positive_int(
    "FACTORY_NO_DIFF_RETRY_RESET_SECONDS", 86400
)
...
def pr_is_static_candidate(
    pr: dict[str, Any], issue_map: dict[int, dict[str, Any]]
) -> bool:
...
def pr_suppresses_issue_candidate(
    ...
    head = str(pr.get("headRefName") or "")
    return str(pr.get("state") or "OPEN").upper() == "OPEN" and head.startswith(
        "factory/"
    )
...
def order_candidates_for_worker(
    candidates: list[Candidate], worker: str
) -> list[Candidate]:
...
def lease_is_stale(
    ...
    *,
    ...
    has_unresolved_active_runs: bool,
