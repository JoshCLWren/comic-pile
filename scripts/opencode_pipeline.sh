...
_is_valid_model() {
	...
	if echo "$model" | grep -qiE "(^openrouter/|^opencode/|^opencode-go/|^anthropic/|^github-copilot/|^mistralai/|^mistral/|^nvidia/mistralai/|^nvidia/mistral/|^mistralai/mistral-small-3\.1-24b-instruct:free)"; then
		return 1
	fi
	return 0
}
  ...
  local candidate_models=()
   while IFS= read -r model; do
     candidate_models+=("$model")
       ...
       | sed 's/^[[:space:]]*//' \
| grep -vE "^openrouter/|^opencode/|^opencode-go/|^anthropic/|^github-copilot/|^mistralai/|^mistral/|^nvidia/mistralai/|^nvidia/mistral/|^mistralai/mistral-small-3\.1-24b-instruct:free|^mistral-small-3\.1-24b-instruct:free" \
       | grep -v "^$" || true)
