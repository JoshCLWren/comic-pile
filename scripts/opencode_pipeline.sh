...
if [[ -f "$LOG_DIR/model_tool_test_results.txt" ]]; then
  while IFS= read -r model; do
        ...
        if echo "$model" | grep -qE "^openrouter/|^opencode/|^opencode-go/|^anthropic/|^github-copilot/|^nvidia/mistralai/|^mistral/|^nvidia/|^deepseek/"; then
            _CODING_POOL+=("$model")
        fi
...
IMPLEMENT_MODELS=()
if [[ -n "${IMPLEMENT_MODEL:-}" ]] && echo "$IMPLEMENT_MODEL" | grep -qE "^openrouter/|^opencode/|^opencode-go/|^anthropic/|^github-copilot/|^nvidia/mistralai/|^mistral/|^nvidia/"; then
  IMPLEMENT_MODELS+=("$IMPLEMENT_MODEL")
fi
IMPLEMENT_MODELS+=("${_CODING_POOL[@]}")

REVIEW_MODELS=()
if [[ -n "${REVIEW_MODEL:-}" ]] && echo "$REVIEW_MODEL" | grep -qE "^openrouter/|^opencode/|^opencode-go/|^anthropic/|^github-copilot/|^nvidia/mistralai/|^mistral/|^nvidia/"; then
  REVIEW_MODELS+=("$REVIEW_MODEL")
fi
REVIEW_MODELS+=("${_CODING_POOL[@]}")

FIX_MODELS=()
    if [[ -n "${FIX_MODEL:-}" ]] && echo "$FIX_MODEL" | grep -qE "^openrouter/|^opencode/|^opencode-go/|^anthropic/|^github-copilot/|^nvidia/mistralai/|^mistral/|^nvidia/|^deepseek/"; then
  FIX_MODELS+=("$FIX_MODEL")
fi
FIX_MODELS+=("${_CODING_POOL[@]}")

PR_MODELS=()
    if [[ -n "${PR_MODEL:-}" ]] && echo "$PR_MODEL" | grep -qE "^openrouter/|^opencode/|^opencode-go/|^anthropic/|^github-copilot/|^nvidia/mistralai/|^mistral/|^nvidia/|^deepseek/"; then
  PR_MODELS+=("$PR_MODEL")
fi
PR_MODELS+=("${_MODEL_POOL[@]}")

CI_CHECK_MODELS=()
    if [[ -n "${CI_CHECK_MODEL:-}" ]] && echo "$CI_CHECK_MODEL" | grep -qE "^openrouter/|^opencode/|^opencode-go/|^anthropic/|^github-copilot/|^nvidia/mistralai/|^mistral/|^nvidia/|^deepseek/"; then
