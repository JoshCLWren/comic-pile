...
## MANDATORY: Frontend Visual Policy
...
`frontend/AGENTS.md` may provide additional scoped guidance, but it is supplemental. **The root instruction above applies to every factory and agent working from the repository root.**
...
Scheduled workflow: `.github/workflows/factory-model-discovery.yml` (also `workflow_dispatch`). It opens `factory/model-retirement` when pins disappear from the live CLI catalog, when sticky `#1093` NVIDIA `factory-model-retired-410:v1` markers match a pin, **or** when unused free OpenCode models (`cost==0`, not paid Zen) are missing from `.github/free-model-factories.tsv`. Surplus `big-pickle` slots convert first; unused `big-pickle` is never grown as a first pin. Adds are on by default for the schedule. It does not auto-add paid Zen models or silently re-pin `retired_models` / 410-locked ids. NVIDIA catalog adds are out of scope. `EXPECTED_WORKERS` is generated in `.github/factory-expected-workers.json` by the apply path. Discovery and `free-model-factory-run.yml` pin the same OpenCode CLI release.
